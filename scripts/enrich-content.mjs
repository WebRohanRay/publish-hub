// ==============================================================================
// NoxWire Content Engine — Immersive, Story-Driven Journalism & Organic SEO
// ==============================================================================

import fs from "fs";
import path from "path";

const seedFilePath = path.resolve(process.cwd(), "src", "data", "seedData.ts");
...
// Read existing seedData.ts to preserve interfaces and categories
const seedFileContent = fs.readFileSync(seedFilePath, "utf-8");
const interfaceBoundary = seedFileContent.indexOf("export const INITIAL_POSTS: Post[] = [");

if (interfaceBoundary !== -1) {
  const topPart = seedFileContent.substring(0, interfaceBoundary);
  const bottomPart = `export const INITIAL_POSTS: Post[] = ${JSON.stringify(postsData, null, 2)};\n\nexport const INITIAL_COMMENTS: CommentItem[] = [
  {
    id: "comm-1",
...
`;
  ...
  fs.writeFileSync(seedFilePath, topPart + bottomPart, "utf-8");
  console.log(`✓ Successfully updated ${seedFilePath} with 20 story-driven, immersive articles!`);
} else {
  console.error("Could not find INITIAL_POSTS boundary in seedData.ts");
}
